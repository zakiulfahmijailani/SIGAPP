...
import MacroAgentAnalysisCard from "@/components/agents/MacroAgentAnalysisCard";
import MacroReportAgentCard from "@/components/agents/MacroReportAgentCard";
import MacroEmailAgentCard from "@/components/agents/MacroEmailAgentCard";
import RegionalBriefingPanel from "@/components/agents/RegionalBriefingPanel";
...
export default function MacroInsightPage() {
  ...
  const semesterLabel = SEMESTERS.find(s => s.key === selectedSemester)?.label || selectedSemester;

  return (
    <>
      <AgentStatusBar mode="macro" />
      ...
      <div className="p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <select 
              ...
              onChange={(e) => {
                ...
                setReportGenerated(false); // Reset on semester change
                setEmailSent(false);
              }}
            >
              {SEMESTERS.map(sem => (
        ...
        {/* 2. MACRO AGENT ANALYSIS CARD */}
        <MacroAgentAnalysisCard 
        ...
        {/* 5. AGENT ACTION CARDS (Report & Email) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <MacroReportAgentCard 
            semesterLabel={semesterLabel}
            totalKecamatan={stats.totalKec}
      ...
      {/* 8. SLIDE-IN PANEL (Conditionally Rendered) */}
      {selectedKecamatan && selectedKecData && (
        <RegionalBriefingPanel 
          kecamatan={selectedKecData}
          allSemesterData={MACRO_INSIGHT_KECAMATAN_DATA}
          nttAverage={stats.avgIndex}
